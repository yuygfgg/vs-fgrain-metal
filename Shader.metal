...
int my_rand_poisson(thread noise_prng &p, float lambda, float prod) {
    ...
    while (u > sum && x < 10000.0f * lambda && x < max_iterations) {
        x += 1.0f;
        prod *= lambda / x;
...
// 修改后的 render_pixel 函数，加入 stride 逻辑
float render_pixel(
    ...
    float inv_grain_radius_mean = 1.0f / grain_radius_mean;
    float ag = 1.0f / ceil(inv_grain_radius_mean);

    // 使用 stride 计算索引，确保正确访问纹理
    float originalPixelValue = src.read(uint2(x % stride, y)).r;  // 使用 stride 处理源纹理

    int index = max(0, min(int(originalPixelValue * 255.1f), 255));
